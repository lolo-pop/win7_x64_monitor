...
void UnInjectAll()
{
	...
	while (bmore)
	{
		...
		int t1=strcmp(pe32.szExeFile,"UnloadDll64.exe");
		if(t1==0){
			check=1;
		}
		if (check==0)
		{
			...
			UnInjectDll("MonDll64.dll",pe32.th32ProcessID);
		//	Sleep(500);
		//	UnInjectDll("EasyHook64.dll",pe32.th32ProcessID);	
		}
		bmore = Process32Next(hprocesssnap , &pe32);
	}
	//f.close();
