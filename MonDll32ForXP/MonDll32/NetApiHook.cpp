...
)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<HttpSendRequest>,func_params=<lpszHeaders|"<<lpFileNamestr1<<">";

	string s,sm;
	sm=logstream.str();
	WriteLog(sm);
	return (realHttpSendRequest)(hRequest, lpszHeaders, dwHeadersLength, lpOptional, dwOptionalLength);
}
...
)
{
	stringstream logstream;
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<HttpSendRequestEx>";

	string s,sm;
	sm=logstream.str();
	WriteLog(sm);
	return (realHttpSendRequestEx)(hRequest, lpBuffersIn, lpBuffersOut, dwFlags, dwContext);
}
...
)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<InternetConnect>,func_params=<lpszServerName|"<<lpFileNamestr1<<",lpszUsername|"<<lpFileNamestr2<<",lpszPassword|"<<lpFileNamestr3<<">";

	string s,sm;
	sm=logstream.str();
	WriteLog(sm);
	return (realInternetConnect)(hInternet, lpszServerName, nServerPort, lpszUsername, lpszPassword, dwService, dwFlags, dwContext);
}
