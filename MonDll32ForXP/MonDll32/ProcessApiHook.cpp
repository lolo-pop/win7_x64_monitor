	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<CreateFileMapping>,func_params=<hFile|"<<hFilepath<<",lpName|"<<lpNamestr<<">";

	string s;
	s=logstream.str();
	WriteLog(s);
	return (realCreateFileMapping)(hFile, lpFileMappingAttributes, flProtect, dwMaximumSizeHigh, dwMaximumSizeLow, lpName);
}

HANDLE WINAPI MyOpenFileMapping( _In_ DWORD dwDesiredAccess, _In_ BOOL bInheritHandle, _In_ LPCWSTR lpName ){
	stringstream logstream;
	string lpNamestr="NULL";
	if (lpName!=NULL)
	{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<OpenFileMapping>,func_params=<dwDesiredAccess|"<<dwDesiredAccess<<",bInheritHandle|"<<bInheritHandle<<",lpName|"<<lpNamestr<<">";

	string s;
	s=logstream.str();
	WriteLog(s);
	return (realOpenFileMapping)(dwDesiredAccess,bInheritHandle,lpName);
}
	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<FindWindowEx>,func_params=<lpszClass|"<<lpFileNamestr1<<",lpszWindow|"<<lpFileNamestr2<<">";

	string s;
	s=logstream.str();
	WriteLog(s);
	return (realFindWindowEx)(hwndParent, hwndChildAfter, lpszClass, lpszWindow);
}
	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<LoadLibraryEx>,func_params=<lpLibFileName|"<<lpFileNamestr<<",dwFlags|"<<dwFlags<<">";
	string s;
	s=logstream.str();
	WriteLog(s);
	return (realLoadLibraryEx)(lpLibFileName, hFile, dwFlags);
}
	...
	)
{
	...
	//{
	BOOL check1=TRUE;
	check1=InjectDll(easyhook32path,pid);
	Sleep(200);
	if (check1==TRUE)
	{
		InjectDll(mondll32path,pid);
	}
	/*
	}else if (check==0)//64位
	{
