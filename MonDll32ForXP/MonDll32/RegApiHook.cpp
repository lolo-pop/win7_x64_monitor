	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<RegOpenKeyEx>,func_params=<hKey|"<<str<<",lpSubKey|"<<lpFileNamestr<<">";
	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<RegSetValueExW>,func_params=<hKey|"<<str<<"lpValueName|"<<lpFileNamestr<<">";
	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<RegRestoreKey>,func_params=<hKey|"<<str<<",lpFile|"<<lpFileNamestr<<">";

	string s,sm;
	sm=logstream.str();
	WriteLog(sm);
	return (realRegRestoreKey)(hKey, lpFile, dwFlags);
}

LONG WINAPI MyRegReplaceKey(
	_In_     HKEY    hKey,
	_In_opt_ LPCWSTR lpSubKey,
	...
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<RegReplaceKey>,func_params=<hKey|"<<str<<",lpSubKey|"<<lpFileNamestr1<<",lpNewFile|"<<lpFileNamestr2<<",lpOldFile|"<<lpFileNamestr3<<">";

	string s,sm;
	sm=logstream.str();
	WriteLog(sm);
	return (realRegReplaceKey)(hKey, lpSubKey, lpNewFile, lpOldFile);
}

LONG WINAPI MyRegLoadKey(
	_In_     HKEY    hKey,
	_In_opt_ LPCWSTR lpSubKey,
	_In_     LPCWSTR lpFile
	)
{
	...
	logstream.clear();
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<RegLoadKey>,func_params=<hKey|"<<str<<",lpSubKey|"<<lpFileNamestr1<<",lpFile|"<<lpFileNamestr2<<">";

	string s,sm;
	sm=logstream.str();
	WriteLog(sm);
	return (realRegLoadKey)(hKey, lpSubKey, lpFile);
}
