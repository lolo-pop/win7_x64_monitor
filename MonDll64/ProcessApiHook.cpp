...
{
	//OutputDebugStringA("openprocess reach");
	stringstream logstream;
	logstream.clear();
	...
	)
{
	...
	if (mypid==tarpid)
	{
		...
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<SetThreadContext>,func_params=<hThread|"<<threadid<<",ProcessPath|"<<GetProPath(tarpid)<<",tarpid|"<<tarpid<<">";
	}
	...
	)
{
	...
	/*if (check==0)//64位进程
	{
		InjectModuleToProcessById(easyhook64path,pid);
		//Sleep(200);
		InjectModuleToProcessById(mondll64path,pid);
	...
	)
{
	...
	if (mypid==tarpid)
	{
		...
		//OutputDebugStringA("TerminateThread string!");
