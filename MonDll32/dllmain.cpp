...
{  
	...
	if(realResumeThread == NULL)
		OutputDebugStringA("realresumethread failed!");
	else
		OutputDebugStringA("realresumethread success!");
	...
	{
		OutputDebugStringA("realsuspenthread failed!");
	}
	else {
		OutputDebugStringA("realsuspenthread success!");
	}
	...
	realVirtualAllocEx= (ptrVirtualAllocEx)GetProcAddress(hKernelBase,"VirtualAllocEx");
	...
	if(realTerminateThread == NULL)
		OutputDebugStringA("realterminatethread failed!");
	else 
		OutputDebugStringA("realterminatethread success!");
	realVirtualFree= (ptrVirtualFree)GetProcAddress(hKernel32,"VirtualFree");
	if(realVirtualFree == NULL)
		OutputDebugStringA("realvirtualfree failed!");
	else 
		OutputDebugStringA("realvirtualfree success!");
...
{  
	...
	//OutputDebugString(L"hahaha1\n");
	//OutputDebugStringA(s.c_str());
	//OutputDebugStringA("\n");
	int dirPos = s.find_last_of('\\');
	//OutputDebugString(L"hahaha2\n");
	...
	if (dirPos != -1)
	{
		...
		for(TiXmlElement* elem = root->FirstChildElement(); elem != NULL; elem = elem->NextSiblingElement())
		{
			...
			if (strcmp(procName.c_str(),"SysLogUpload.exe") == 0 
				...
				|| strcmp(procName.c_str(), "lsass.exe") == 0
				)
			{
				//cout<<procName.c_str()<<endl;
				api[count++] = 0;
				continue;
			}
	...
	*/
	...
	{
		OutputDebugStringA("11111111111111111111111111111111111111111111111111");
		NTSTATUS result = LhInstallHook(realResumeThread,MyResumeThread,NULL,hHookResumeThread);
		if(FAILED(result))
		{
			switch (result)
		    {
		        ...
		        default:{
			        WCHAR dbgstr[512] = {0};
			        wsprintf(dbgstr, L"%d\n", statue);
			        OutputDebugString(dbgstr);break;
						}
			}
			OutputDebugStringA("install 32 hook resumethread fail");
		}
		else
			OutputDebugStringA("install 32 hook resumethread success");
	...
	{
		NTSTATUS result = LhInstallHook(realSuspendThread,MySuspendThread,NULL,hHookSuspendThread);
		if(FAILED(result))
		{
			switch (result)
		    {
			...
			OutputDebugStringA("install 32 hook suspendthread fail");
		}
	...
	{
		NTSTATUS result = LhInstallHook(realTerminateThread,MyTerminateThread,NULL,hHookTerminateThread);
		if(FAILED(result))
		{
			switch (result)
		    {
			...
			OutputDebugStringA("install 32 hook terminatethread fail");
		}
		LhSetExclusiveACL(HookTerminateThread_ACLEntries, 1, hHookTerminateThread);
	}
	if (api[82]==1&&realVirtualFree!=NULL)
	{
		NTSTATUS result = LhInstallHook(realVirtualFree,MyVirtualFree,NULL,hHookVirtualFree);
		if(FAILED(result))
		{
			switch (result)
		    {
			...
			OutputDebugStringA("install 32 hook virtualfree fail");
		}
...
{  
    ...
    if (api[13]==1&&realCreateFileMappingW!=NULL)
	{
...
{  
	...
	case DLL_PROCESS_ATTACH:  
		{  
			...
			strcpy_s(dlldir,str2.c_str());
		//  MemoryModule无法获取Module的路径
		//	strcpy_s(dlldir,"C:\\Users\\User\\Desktop\\项目\\SVN\\new_UserMonitor\\Zhangmingxing\\Debug\\");
		//	cout<<dlldir<<endl;
			OutputDebugStringA("\n");
			...
			sprintf_s(log_path,"%sLog.txt",dlldir);//获取日志路径
			//strcpy(log_path,"C:\\Log\\Log.txt");
			strcpy_s(spy,"00000");//获取监测层
