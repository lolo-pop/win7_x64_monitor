	...
	)
{
	...
	if (dwProcessId != 0)
	{
		...
		buf = " ";
	logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<OpenProcess>,func_params=<dwDesiredAccess|"<<dwDesiredAccess<<",bInheritHandle|"<<bInheritHandle<<",dwProcessId|"<<dwProcessId<<",proc_name|"<<buf<<">";
	...
	)
{
	...
	if (mypid == tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<QueueUserAPC>,func_params=<hThread|"<<threadid<<",ProcessPath|NULL"<<">";
	} else {
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<QueueUserAPC>,func_params=<hThread|"<<threadid<<",ProcessPath|"<<GetProPath(tarpid)<<",tarpid|"<<tarpid<<">";
	}
	string s;
	s=logstream.str();
	WriteLog(s);
	...
	)
{
	...
	int mypid = _getpid();
	int tarpid = GetProcessId(hProcess);
	logstream.clear();
	if(mypid == tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<ReadProcessMemory>,func_params=<hProcess|"<<buf<<",ProcessPath|NULL"<<",tarpid|"<<tarpid<<">";
	}
	else {
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<ReadProcessMemory>,func_params=<hProcess|"<<buf<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
	...
	)
{
	...
	int threadid=-1;
	threadid=GetThreadId(hThread);//XP下没有这个API
	//根据线程句柄，获取线程句柄所在进程的进程号和进程路径
	logstream.clear();
	int mypid=_getpid();
	int tarpid=GetProcessIdOfThread(hThread);
	HANDLE hProcess=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ, FALSE, tarpid);
	TCHAR path1[MAX_PATH];
	string buf;
	if (tarpid != 0)
	{
	...
	if (mypid==tarpid)
	{
		...
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<ResumeThread>,func_params=<hThread|"<<threadid<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
	...
	)
{
	...
	if (mypid==tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<SetThreadContext>,func_params=<hThread|"<<threadid<<",ProcessPath|NULL"<<">";
	}else{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<SetThreadContext>,func_params=<hThread|"<<threadid<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
	...
	)
{
	...
	logstream.clear();
	int mypid=_getpid();
	DWORD tarpid=GetProcessIdOfThread(hThread);
	HANDLE hProcess=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ, FALSE, tarpid);
	TCHAR path1[MAX_PATH];
	string buf;
	if (tarpid!=0)
	...
	if (mypid==tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<SuspendThread>,func_params=<hThread|"<<threadid<<",ProcessPath|NULL"<<">";
	}else{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<SuspendThread>,func_params=<hThread|"<<threadid<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
	...
	)
{
	...
	if(mypid == tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<VirtualProtectEx>,func_params=<hProcess|"<<buf<<",flNewProtect|"<<flNewProtect<<",lpflOldProtect|"<<lpflOldProtect<<",ProcessPath|NULL"<<">";

	}
	else {
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<VirtualProtectEx>,func_params=<hProcess|"<<buf<<",flNewProtect|"<<flNewProtect<<",lpflOldProtect|"<<lpflOldProtect<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	...
	)
{
	...
	if(mypid == tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<WriteProcessMemory>,func_params=<hProcess|"<<buf<<",ProcessPath|NULL"<<",tarpid|"<<tarpid<<">";
	}
	else {
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<WriteProcessMemory>,func_params=<hProcess|"<<buf<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
	string s;
	s=logstream.str();
	WriteLog(s);
	...
	)
{
	...
	sprintf_s(exe1,"%sMonInject64.exe %sEasyHook64.dll %d",dlldir,dlldir,pid);
	sprintf_s(exe2,"%sMonInject64.exe %sMonDll64.dll %d",dlldir,dlldir,pid);
	if (check==1)//32位进程
	{
		InjectDll(easyhook64path,pid);
		//Sleep(200);
		InjectDll(mondll64path,pid);
	}else if (check==0)//64位进程
	{
	...
	)
{
	...
	if (mypid==tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<TerminateThread>,func_params=<hThread|"<<threadid<<",ProcessPath|NULL"<<">";
	}else{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<TerminateThread>,func_params=<hThread|"<<threadid<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
	...
	)
{
	...
	TCHAR path1[MAX_PATH];
	string buf;
	GetProcessFullPath(lpAddress,path1);
	buf=WideToMutilByte(path1);
	int mypid = _getpid();
	int tarpid = GetProcessId(lpAddress);
	logstream.clear();
	if(mypid == tarpid)
	{
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<VirtualFree>,func_params=<dwFreeType|"<<dwFreeType<<",ProcessPath|NULL"<<",tarpid|"<<tarpid<<">";
	}
	else {
		logstream<<hostname<<" "<<spy<<" "<<LogTime()<<"account_name="<<"<"<<strBuffer<<">,proc_id=<"<<_getpid()<<">,proc_name=<"<<ProcessName<<">,proc_thread_id=<"<<GetCurrentThreadId()<<">,proc_img_path=<"<<GetProcessPath()<<">,proc_func_name=<VirtualFree>,func_params=<dwFreeType|"<<dwFreeType<<",ProcessPath|"<<buf<<",tarpid|"<<tarpid<<">";
	}
